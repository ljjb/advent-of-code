...
let calibration int_line =
  let fold_f (left, _) ele =
    let left' = match left with None -> Some ele | _ -> left in
...
let filter_part2 line =
  ...
  let map =
    List.fold subs ~init ~f:(fun acc (word, digit) ->
        ...
        |> List.fold ~init:acc ~f:(fun acc idx ->
               Map.set acc ~key:idx ~data:digit))
  in
  String.mapi line ~f:(fun i c ->
      match Map.find map i with None -> c | Some c -> c)
  |> filter_part1

let solve filename ~filter =
  let f ic =
    Util.read_lines ic |> Sequence.map ~f:filter
